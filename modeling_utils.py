# Python script to for modeling utilitie functions
# Author: Karthik D
...
def get_model_metrics(model, x, y):
    ...
    #met = pd.DataFrame(classification_report(y_act_class, y_pred))
    # met = met.drop("macro avg", axis=1).drop('weighted avg', axis=1).T
    #return met.drop('support', axis=1), y_pred, y_scores
    return classification_report(y_act_class, y_pred)