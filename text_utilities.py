# Python script for text utility functions
# Author: Karthik D
...
def clean_string(sent):
    ...
    return ' '.join(filter(str.isalpha, sent))
...
def make_tensor_torch(data, w2i, max_len):
    if type(data) == str:
        ret_data = [padding([w2i[word] for word in data.split() if word in w2i], max_len, w2i[PAD])]
    else:
        #seq_len = max(data.str.split().apply(len))
        ret_data = [padding([w2i[word] for word in row.split() if word in w2i], max_len, w2i[PAD]) for row in data]
